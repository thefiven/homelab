...
The cap itself is not this ADR's decision: ADR-0002 already fixed observability's
envelope at 3 GiB, roughly 9% of the 32 GiB ceiling. What remains is which stack
runs inside that envelope, how the envelope splits between its parts, and which
few alerts are worth waking someone for. Findings throughout are from
...
headroom rule (50% of free RAM as the OOM-avoidance margin). Against Prometheus
...
| Unallocated margin | 178 MiB |

Removing the separate collector that a Prometheus-based stack would have
needed freed the budget this split spends instead on vmalert and Alertmanager.
Every figure is a cgroup limit (`systemd` `MemoryMax=` or a container memory
limit), not an internal setting: VictoriaMetrics states plainly that
