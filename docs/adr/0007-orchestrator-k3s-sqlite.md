...
request" — no passthrough, no dedicated VM. That presupposes something can
actually schedule the GPU across workloads, which rules out one candidate
...
workloads to the GPU host by hand, forever — the opposite of #28. Komodo also
requires a MongoDB or FerretDB dependency with a documented memory-default
hazard (unpinned WiredTiger cache defaults to ≈50% of RAM − 1 GB, ≈15.5 GB on
this machine), publishes no support window, and ships under a "no warranties"
GPL-3.0 README. Rejected.
...
family with a mature, first-party, pull-based GitOps ecosystem (Flux) that
fits the no-CI-secret constraint exactly. Selected as the family; which
distribution is the next question.

## k3s, not k0s or kubeadm

The ticket names the cost explicitly — "annual maintenance," "breaking
changes," "debugging at 22:00 by one person" — which is an optimization
target for low-touch operation, not for staying closest to the reference
implementation.
...
**k3s.** The only one of the five with **documented automatic certificate
renewal**: certificates within 120 days of expiring renew automatically on
every k3s start, with a Kubernetes Warning Event emitted if one is
approaching. For a single operator on a 24/7 box that may not be touched for
months, this removes a whole class of self-inflicted outage that both
kubeadm and (undocumented) k0s carry. Adding a worker is one command
(`curl ... | K3S_URL=... K3S_TOKEN=... sh -`) regardless of node count or
hardware generation. Its own weaknesses — no formally published support
...
own published test — are the mildest of the three. Selected.
...
and no embedded-etcd files exist on disk — this is a decision to not
...
Additional machines join as k3s agents (workers) as the service set grows;
the server role is not replicated. Release channel: `stable`.

## Alternatives rejected

**Docker Compose + Komodo/Dockge.** No real scheduler — contradicts #28's
GPU-as-platform-resource decision. Komodo's undocumented Mongo memory default
is a standing hazard on a 32 GB machine; Dockge has no git story at all.
...
- **k3s ships bundled components** (Traefik ingress, ServiceLB,
  local-path-provisioner, Flannel CNI, CoreDNS) by default. #19 (storage
  layout) and #22 (public exposure and ingress) each decide, independently,
  whether to keep or `--disable` the ones in their domain.
...
- If the growth plan ever calls for real control-plane redundancy across
  ...
  forever" finding — converting to embedded etcd is a single
  `--cluster-init` restart, with feasibility already validated by #30.
